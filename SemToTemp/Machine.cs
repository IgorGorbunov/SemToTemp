...
/// <param name="docYear">Год документа</param>
    public Machine(string name, string title, GroupElement groupElement, Dictionary<string, string> parametrs, string doc, string docYear)
    :base(name, title, groupElement, parametrs, doc, docYear)
    {
        AddSqlPosParam();
        Stype = ((int)ElementType.Machine).ToString();
